...
image_tag_tmp=${ref_value//\//-} ## replace `/` with `-` in ref for docker tag requirement (master or 2019-03-13)
...
if [ "${ref_type}" = "tags" ]
then
  docker tag $IMAGE_NAME $REGISTRY_IMAGE:$image_tag_tmp
  docker push $REGISTRY_IMAGE:$image_tag_tmp
fi
